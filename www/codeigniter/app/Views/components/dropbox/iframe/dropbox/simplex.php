<?php
// if(!file_exists("../../application.php")) die("ERROR.");
// include("../../application.php");
// //error_log($CFG->sesion);
...
//
$CONFIG['confHiddenFile'] = array(".ftpquota", "index.php", "index.php~", ".index.php.swp", ".htaccess", ".htpasswd", "simplex.php", "error_log");
...
//
$CONFIG['confEnableUpload'] = false;
$CONFIG['confEnableMkDir'] = false;
$CONFIG['confEnableDelete'] = false;
$CONFIG['confEnableDownload'] = true;
...
//
// $CONFIG['confStartDir'] = "./editable/$ruta/";
...
//
//$ruta = $_GET['ruta'];
// $CONFIG['confBaseDir'] = "./css/";
// $CONFIG['confBaseDir'] = "./editable/$ruta/";
...
class GateKeeper
{
    ...
    public static function isDownloadAllowed()
    {
        if (Simplex::getConfig("confEnableDownload") == true) return true;
        return false;
    }
...
class FileManager
{
    ...
    public static function download_dir($dir)
    {
//			error_log("Comprimiendo directorio $dir ...");

			$temp_file = tempnam(sys_get_temp_dir(), 'DWNLD');
//			error_log($temp_file);
			$command="zip -r " . $temp_file . " \"" . $dir . "\"";
//			error_log($command);
			exec($command);
			$fileName = basename($dir) . ".zip"; 
			$fileSize = filesize($temp_file . ".zip");

			header("Content-Type: application/zip");
			header("Content-Length: ".$fileSize);
			header("Content-Disposition: attachment; filename=\"".$fileName."\"");

			readfile_chunked($temp_file . ".zip"); 
			unlink($temp_file);
    ...
    function run($location)
    {
        ...
        if (isset($_GET['download'])) {
            if (GateKeeper::isDownloadAllowed()) {
                $split_path = Location::splitPath($_GET['download']);
                $path = "";
...
class Simplex
{
    ...
    function makeLink($logout, $sort_by, $sort_as, $delete, $dir, $download=null)
    {
        $link = "?";
        if ($logout == true) {
            $link .= "logout";
        ...
        if ($delete != null) $link .= "&amp;del=" . $delete;
        return $link;
    }
    ...
    {
        global $ERROR;
        if (isset($ERROR) && strlen($ERROR) > 0);// keep the first error and discard the rest
        else
            $ERROR = $message;
    }
    ...
    ?>
            <div class="form-group">
                ...
                <input type="text" name="inputUsername" id="inputUsername" class="form-control"
                    placeholder="<?php print $this->getString("langInputUsername"); ?>" required>
            </div>
            <?php
...
?>
            <div class="form-group">
                ...
                <input type="password" name="inputPassword" id="inputPassword" class="form-control"
                    placeholder="<?php print $this->getString("langInputPassword"); ?>" required>
            </div>
            <button class="btn btn-default hidden-xs"
                type="submit"><?php print $this->getString("langBtnSignin"); ?></button>
            <button class="btn btn-default btn-block visible-xs"
                type="submit"><?php print $this->getString("langBtnSignin"); ?></button>
...
<head>
    ...
    @font-face {
        font-family: 'NouvelR';
        src: url('../../css/fonts/NouvelR-Book.woff2') format('woff2');
        font-weight: 300;
        font-style: normal;
    }
    ...
    @font-face {
        font-family: 'NouvelR';
        src: url('../../css/fonts/NouvelR-Regular.woff2') format('woff2');
        font-weight: 400;
        font-style: normal;
    }
    ...
    @font-face {
        font-family: 'NouvelR';
        src: url('../../css/fonts/NouvelR-Semibold.woff2') format('woff2');
        font-weight: 600;
        font-style: normal;
    }
        ...
        <div class="container">
            <div class="row">
                <h3 class="no-strong text-center">
                    <a
                        href="<?php print $this->makeLink(false, null, null, null, ""); ?>"><span><?php if (Simplex::getConfig('confTitle') != null) print Simplex::getConfig('confTitle'); ?></span></a>
                    <?php
    			...
    			?>
                    <span><small
                            class="no-strong"><?php print $confTitleSub[array_rand($confTitleSub)]; ?></small></span>
                    <?php
				}
				?>
                </h3>
        ...
        <div class="row">
            <div class="col-xs-12">
                <p>
                    <?
			$ruta_actual = getcwd();
			$directorio_padre = basename($ruta_actual);
				?>
                    <a href="?dir=/#simplex"><?= $directorio_padre ?> </a> /
                    <?php
					for ($i = 0; $i < count($this->location->path); $i++) {
    		?>
                    <a
                        href="<?php print $this->makeLink(false, null, null, null, $this->location->getDir(false, true, false, count($this->location->path) - $i - 1)); ?>#simplex"><?php print $this->location->getPathLink($i, true); ?></a>
                    /
                    <?php
        ...
        <table class="table table-hover">
            <thead class="small">
                <tr>
                    ...
                    <th class="no-strong text-center hidden-xs" style="width: 156px;">
                        <?php print $this->makeArrow("mod"); ?></th>
                    <th class="no-strong text-center" style="width: 97.5px;"><?php print $this->makeArrow("size"); ?>
                    </th>
                    <?php if (GateKeeper::isDownloadAllowed()) { ?>
                    <th class="no-strong text-center" style="width: 39px;">
                        <?php print Simplex::getString("langFileDownload"); ?></th>
                    <?php } ?>
                    <?php if (GateKeeper::isDeleteAllowed()) { ?>
                    <th class="no-strong text-center" style="width: 39px;">
                        <?php print Simplex::getString("langFileDelete"); ?></th>
                    <?php } ?>
                </tr>
            </thead>
            ...
            <tbody>
                <tr>
                    ...
                    <td class="text-left"><a
                            href="<?php print $this->makeLink(false, null, null, null, $this->location->getDir(false, true, false, 1)); ?>#simplex">...</a>
                    </td>
    ...
    foreach ($this->dirs as $dir) {
        ?>
                ...
                <tr>
                    ...
                    <td class="text-left">
                        <a
                            href="<?php print $this->makeLink(false, null, null, null, $this->location->getDir(false, true, false, 0) . $dir->getNameEncoded()); ?>/#simplex"><?php print $dir->getNameHtml(); ?></a>
                    </td>
                    <td class="hidden-xs"></td>
                    <td></td>
                    <?php if (GateKeeper::isDownloadAllowed()) { ?>
                    <td class="text-center download"><a data-name="<?php print htmlentities($dir->getName()); ?>"
                            href="<?php print $this->makeLink(false, null, null, null, $this->location->getDir(false, true, false, 0), $this->location->getDir(false, true, false, 0) . $dir->getNameEncoded()); ?>"
                            onClick="logFileClick('<?php print $this->location->getDir(false, true, false, 0) . $dir->getNameEncoded(); ?>','dir')"
                            download><img src="?img=browser_download"
                                alt="<?php print $this->getString("langInfoDownload"); ?>"></a></td>
                    <?php } ?>
                    <?php if (GateKeeper::isDeleteAllowed()) { ?>
                    <td class="text-center delete"><a data-name="<?php print htmlentities($dir->getName()); ?>"
                            href="<?php print $this->makeLink(false, null, null, $this->location->getDir(false, true, false, 0) . $dir->getNameEncoded(), $this->location->getDir(false, true, false, 0)); ?>"><img
                                src="?img=del" alt="<?php print $this->getString("langInfoDelete"); ?>"></a></td>
                    <?php } ?>
                </tr>
                <?php
$row = !$row;
...
if ($this->files) {
    ...
    foreach ($this->files as $file) {
        ?>
                ...
                <tr>
                    <td><img alt="<?php print $file->getType(); ?>"
                            src="<?php print $this->makeIcon($file->getType()); ?>"></td>
                    <td class="text-left"><a
                            onClick="logFileClick('<?php print $this->location->getDir(false, true, false, 0) . $file->getNameEncoded(); ?>','file')"
                            href="<?php print $this->location->getDir(false, true, false, 0) . $file->getNameEncoded(); ?>" <?php
    ...
    ?></a></td>
                    ...
                    <td class="download"><a
                            onClick="logFileClick('<?php print $this->location->getDir(false, true, false, 0) . $file->getNameEncoded(); ?>','file')"
                            href="<?php print $this->location->getDir(false, true, false, 0) . $file->getNameEncoded(); ?>"
                            download>
                            <img src="?img=browser_download"
                                alt="<?php print $this->getString("langInfoDownload"); ?>"></a></td>
                    <?php } ?>
                    <?php if (GateKeeper::isDeleteAllowed()) { ?>
                    <td class="delete"><a data-name="<?php print htmlentities($file->getName()); ?>"
                            href="<?php print $this->makeLink(false, null, null, $this->location->getDir(false, true, false, 0) . $file->getNameEncoded(), $this->location->getDir(false, true, false, 0)); ?>">
    ...
    ?>
                <div class="col-xs-12 col-sm-6 col-md-5 col-lg-4 pull-right">
                    <form class="navbar-form navbar-right" enctype="multipart/form-data" method="post">
                        <div class="form-group">
                            <label class="sr-only"
                                for="newDir"><?php print $this->getString("langInfoMkDir"); ?></label>
                            <input class="form-control" name="newDir" id="newDir"
                                placeholder="<?php print $this->getString("langInfoMkDir"); ?>" type="text" required>
                        </div>
                        <button class="btn btn-default hidden-xs"
                            type="submit"><?php print $this->getString("langBtnMkdir"); ?></button>
                        <button class="btn btn-default btn-block visible-xs"
                            type="submit"><?php print $this->getString("langBtnMkdir"); ?></button>
                ...
                <div class="col-xs-12 col-sm-7 col-md-5 pull-right" style="padding-bottom: 11px;">
                    <form class="navbar-form navbar-right" enctype="multipart/form-data" method="post">
                        ...
                        <div class="form-group">
                            <label class="sr-only"
                                for="newFile"><?php print $this->getString("langInfoUpload"); ?></label>
                            <div class="input-group">
                                <span class="input-group-btn">
                                    <span
                                        class="btn btn-default btn-file"><?php print $this->getString("langBtnChooseFile"); ?><input
                                            type="file" name="newFile" id="newFile" multiple></span>
                                </span>
                                <input type="text" class="form-control"
                                    placeholder="<?php print $this->getString("langInfoUpload"); ?>" readonly>
                            </div>
                        </div>
                        <button class="btn btn-default hidden-xs"
                            type="submit"><?php print $this->getString("langBtnUpload"); ?></button>
                        <button class="btn btn-default btn-block visible-xs"
                            type="submit"><?php print $this->getString("langBtnUpload"); ?></button>
    ...
    ?>
                <div class="col-xs-12 col-sm-2 pull-left" style="margin:8px 0 8px 0;">
                    <a href="<?php print $this->makeLink(true, null, null, null, ""); ?>">
                        <button class="btn btn-default hidden-xs"
                            type="button"><?php print $this->getString("langBtnSignout"); ?></button>
                        <button class="btn btn-default btn-block visible-xs"
                            type="button"><?php print $this->getString("langBtnSignout"); ?></button>
                    </a>
                </div>
...
?>
                <div class="row">
                    <div class="col-xs-12 text-center">
                        <h4 class="no-strong">
                            <?php if (Simplex::getConfig('confTitle') != null) print Simplex::getConfig('confTitle'); ?>
                            with <a href="https://github.com/kdzlvaids/Simplex">Simplex</a></h4>
                    </div>
                </div><!-- row -->
            </div><!-- container -->
...
?>
                <div class="row">
                    <div class="col-sm-8 text-left">
                        <!-- <h4 class="no-strong">
<?php if (Simplex::getConfig('confTitle') != null) print Simplex::getConfig('confTitle'); ?> with <a href="https://github.com/kdzlvaids/Simplex">Simplex</a>
</h4> -->
                    </div>
...
<a href="#simplex"><button class="btn btn-default" type="button" style="margin:3px 0 2px 0;">Top</button></a>
    ...
    <?php if (($this->getConfig('confLogFile') != null && strlen($this->getConfig('confLogFile')) > 0) || (GateKeeper::isDeleteAllowed())) { ?>
    $(document).ready(function() {
        <?php if (GateKeeper::isDeleteAllowed()) { ?>
...
if ($this->logging == true) {
    ?>
        ...
        $("a.file").click(function() {
            logFileClick("<?php print $this->location->getDir(true, true, false, 0); ?>" + $(this)
                .html());
            return true;
        });
        <?php
...
?>
    ...
    function logFileClick(path, type) {
        //console.log(path);
        $.ajax({
            url: '../../log.php',
            async: true,
